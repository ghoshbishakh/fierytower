...
class Ball(pygame.sprite.Sprite):
    def __init__(self):
        ...
        Ball.image=pygame.image.load('ball2.png')
...
player2.image=pygame.image.load('full_up_rest2.png')
player2.rect.x = 10
player2.rect.y = 450
...
def scrolldownfunc(degree):
    ...
    player1.rect.y=player1.rect.y+degree
    player2.rect.y = player2.rect.y + degree
a=0
def drawstepsandplayer(block_list,a):
    ...
    qwe2=pygame.sprite.RenderPlain() #the sprite group to use its draw method
    qwe2.add(player2)
    qwe2.draw(screen)
red = (255, 0, 0)   
done=False
...
while done==False:
    ...
    for event in pygame.event.get(): # User did something
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_LEFT:
                player1.changespeed_x(-6)
            if event.key == pygame.K_RIGHT:
                player1.changespeed_x(6)
            if event.key == pygame.K_UP:
                pygame.key.set_repeat(0,0)
                player1.image=pygame.image.load('full_up_jump.png')
                player1.jump(block_list)
                jump.play(2)
            if event.key == pygame.K_DOWN:
                player1.changespeed_y(6)

                
            if event.key == pygame.K_a:
                player2.changespeed_x(-6)
            if event.key == pygame.K_d:
                player2.changespeed_x(6)
            if event.key == pygame.K_w:
                pygame.key.set_repeat(0,0)
                player2.image=pygame.image.load('full_up_jump2.png')
                player2.jump(block_list)
                jump.play(2)
            if event.key == pygame.K_s:
                player2.changespeed_y(6)    
        if event.type == pygame.KEYUP:
            player1.image=pygame.image.load('full_up_rest.png')
            if event.key == pygame.K_LEFT:
                player1.changespeed_x(-0)
            if event.key == pygame.K_RIGHT:
                player1.changespeed_x(0)

            player2.image=pygame.image.load('full_up_rest2.png')
            if event.key == pygame.K_a:
                player2.changespeed_x(-0)
            if event.key == pygame.K_d:
                player2.changespeed_x(0)
    ...
    if player2.rect.x >= 700:
        player2.rect.x = -15
    if player2.rect.x <= -20:
        player2.rect.x = 699
    
    score+=player1.calc_grav(count)
    score+=player2.calc_grav(count)
    player1.update(block_list)
    player2.update(block_list)
    block_list.update()
    ...
    clock.tick(60)
    #time.delay(1)
    if pygame.sprite.spritecollide(player1, gendg, True):
        done=True
        winner = "PLAYER 2"
    if pygame.sprite.spritecollide(player2, gendg, True):
        done=True
        winner= "PLAYER 1"
...
rank=str(rank)
r="Your rank is " +rank
printText(str(winner) + " WON!!", "MS Comic Sans", 30, 230, 220, red)
if norank:
